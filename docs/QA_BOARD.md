...
## Round 2 — auditoria dos próprios patches (Fase 8.3 → 8.3b)

Após aplicar os patches do round 1, segunda varredura encontrou **3 buracos novos** que o primeiro QA não viu porque ainda nem existiam:

### 🐛 Buraco #5 · `runAutopilotForSite` (cron interno) não pré-checa BYOK

`runAutopilotNow` (manual) pré-checa, mas o **cron interno** chama `runAutopilotForSite` direto. Se o site tem provider sem chave, **cada trend do batch** dispara o throw → catch → log do loop. 3 trends = 3 erros idênticos no `notes` do AutopilotRun. Desperdiça query + polui log.

**Fix**: Pré-checagem movida pra dentro de `runAutopilotForSite` — uma única mensagem clara antes do loop. Cron interno agora também se beneficia.

### 🧹 Refactor #6 · `resolveAutopilotUserId` estava private

`actions.ts` reimplementou a lógica (env + admin fallback). Era duplicação.

**Fix**: exportado `resolveAutopilotUserId` e novo helper `checkProviderKey(userId, provider) → null | mensagem` em `src/lib/autopilot/index.ts`. `actions.ts` usa ambos.

### 🐌 Perf #7 · dupla chamada de `collectActiveSources` no manual

`runAutopilotNow` chamava `collectActiveSources({siteId})` + `collectActiveSources({siteId: null})`. Mas `collectActiveSources({})` já pega ambos (siteId undefined no filtro).
...
- 18/06 02:00 — Reconciliação concluída: 4 causas concorrentes, todas reais. Patches 1/2/3 aplicados (commit `a4c4ded` · Fase 8.3). Tabela de feeds vivos validada com curl.
- 18/06 02:15 — Round 2 de auditoria sobre os próprios patches. 3 buracos novos encontrados (cron sem pré-check, helper duplicado, coleta duplicada). Patches 5/6/7 aplicados (Fase 8.3b).
