...
- [ ] CRUD de sites no painel.
- [ ] Modelo de conteúdo: `posts` / `pages` / `media`.
- [ ] Renderização multi-tenant (rota por domínio/subdomínio).
- [ ] Conectar domínio customizado e publicar site no ar.
- [ ] Editor de conteúdo + **publicação manual** (site individual e "geral/todos").
